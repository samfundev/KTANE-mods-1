...
public class TestHarness : MonoBehaviour
{
	...
	public float zoomSpeed = 0.25f;      // Speed of the camera going back and forth
	...
	void PrepareBomb(List<KMBombModule> bombModules, List<KMNeedyModule> needyModules, KMWidget[] widgets)
	{
		...
		List<Transform> timerAnchors = new List<Transform>();

		modules.AddRange(bombModules.Select(x => x.transform));
		modules.AddRange(needyModules.Select(x => x.transform));
		...
		if (bomb != null)
		{
			...
			{
				anchors.AddRange(face.Anchors);
				if (face.TimerAnchors.Count > 0)
					timerAnchors.AddRange(face.TimerAnchors);
				else
					timerAnchors.AddRange(face.Anchors);
			}
			while ((modules.Count + 1) > anchors.Count)
			{
				Transform module = modules[Random.Range(0, modules.Count)];
		...
		else
		{
			...
			int square = 1;
			while ((square * square) < (modules.Count + 1))
				square++;
			...
			for (float i = (-squaresize / 2) + 0.1f; i < squaresize / 2; i += 0.2f)
			{
				for (float j = (-squaresize / 2) + 0.1f; j < squaresize / 2; j += 0.2f)
				{
					...
					anchor.SetParent(bombTransform, true);
					anchors.Add(anchor);
					timerAnchors.Add(anchor);
				}
			}
		}
		anchors = anchors.OrderBy(x => Random.value).ToList();
		Transform timerAnchor = timerAnchors[Random.Range(0, timerAnchors.Count)];
		anchors.Remove(timerAnchor);
		...
		foreach (Transform module in modules)
		{
			...
			Transform anchor = anchors.First();
